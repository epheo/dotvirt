...
// Passwords are generated by default, so they must not widen SSH: with a key
// present the password is console-only; without one it is the only way in.
func TestManifestPasswordSSHAuth(t *testing.T) {
	spec := Spec{
		Name: "pw", Namespace: "team-a",
		Instancetype: "u1.small", Preference: "fedora",
		OSImage:   OSImageRef{Name: "fedora", Namespace: "kv"},
		CloudInit: &CloudInit{User: "admin", Password: "pw", SSHKey: "ssh-ed25519 AAAA"},
	}
	_, content, err := Manifest(spec)
	if err != nil {
		t.Fatalf("Manifest: %v", err)
	}
	if strings.Contains(string(content), "ssh_pwauth") {
		t.Errorf("password with an SSH key must not enable password SSH:\n%s", content)
	}

	spec.CloudInit = &CloudInit{User: "admin", Password: "pw"}
	_, content, err = Manifest(spec)
	if err != nil {
		t.Fatalf("Manifest: %v", err)
	}
	if !strings.Contains(string(content), "ssh_pwauth: True") {
		t.Errorf("password without an SSH key must enable password SSH:\n%s", content)
	}
}
