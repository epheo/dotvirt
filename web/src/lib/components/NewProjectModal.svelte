...
>
	<div class="flex max-h-[90vh] w-full max-w-md flex-col rounded-lg bg-white shadow-xl">
		<header class="flex items-center justify-between border-b border-slate-200 px-5 py-3">
			<h2 class="text-base font-semibold text-slate-800">New Project</h2>
		...
		<div class="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4 text-sm">
			<label class="block">
				...
				/>
				<span class="mt-1 block text-[11px] text-slate-400">Creates the tenant git repo of the same name.</span>
			</label>
			<label class="block">
			...
			<p class="rounded bg-slate-50 px-3 py-2 text-xs text-slate-500">
				Creates the tenant repo now, and stages its first namespace{#if owners.trim()} + an owners
					admin grant{/if} into the platform repo. Applied by Argo on merge — open the PR from
				“Changes”.
			</p>
			{#if error}
		...
		<footer class="flex items-center gap-2 border-t border-slate-200 px-5 py-3">
			...
			>
				{submitting ? 'Staging…' : 'Stage project'}
			</button>
		</footer>
	</div>
</div>
