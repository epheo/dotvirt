...
>
	...
	<dl class="divide-y divide-line-soft border-b border-line-soft text-[13px]">
		<Row label="Host">
			{#if vm.nodeName}<a
		...
		<Row label="Uptime" value={vm.startedAt && vm.power === 'On' ? duration(vm.startedAt) : ''} />
		<Row label="OS" value={vm.os ?? ''} />
	</dl>
	...
	<div class="flex items-center gap-2 px-3 py-1.5">
		...
		{/if}
		{#if stagedItem}
			<Button variant="link" size="sm" class="ml-auto" onclick={() => ui.openChanges()}
				>Review</Button
			>
		{/if}
	</div>
	{#if vm.sourceFile}
		<dl class="divide-y divide-line-soft border-b border-line-soft text-[13px]">
			<Row label="Source">
				<span class="block truncate font-mono text-xs text-ink" title={vm.sourceFile}
					>{vm.sourceFile}</span
				>
			</Row>
		</dl>
	...
	<dl class="divide-y divide-line-soft border-b border-line-soft text-[13px]">
		...
		{/if}
		<Row label="vCPU / Memory" value="{sizing.cpu ?? '—'} / {sizing.memory ?? '—'}" />
		<Row label="Disks">
			{vm.disks?.length ?? 0}
