...
{#snippet segmentCard(net: PortGroup)}
	...
	>
		<div class="flex flex-wrap items-center gap-2">
			...
			<span class="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-ink-muted"
		...
		{#if list.length}
			<div class="flex flex-wrap gap-1 pl-5">
				{#each list.slice(0, 14) as vm (vm.namespace + '/' + vm.name)}
					<span
						class="inline-flex items-center gap-1 rounded bg-slate-50 px-1.5 py-0.5 text-[11px] text-ink-soft"
					>
		...
		{#if uplinks.length}
			<div class="mb-2 flex flex-wrap gap-1.5 pl-6">
				{#each uplinks as u (u.name)}
					<span
						class="rounded border border-slate-200 bg-white px-2 py-0.5 text-[11px] text-ink-soft"
					>
