...
>
	<div class="flex w-full max-w-md flex-col rounded-lg bg-white shadow-xl">
		<header class="flex items-center justify-between border-b border-slate-200 px-5 py-3">
			<h2 class="text-base font-semibold text-slate-800">Add Uplink</h2>
		...
		<div class="space-y-4 px-5 py-4 text-sm">
			...
			<p class="rounded bg-amber-50 px-3 py-2 text-xs text-amber-700">
				...
				network — it changes node networking, so review the PR carefully. Cluster-scoped — proposed to
				the platform repository. Requires the NMState operator.
			</p>
			{#if error}
		...
		<footer class="flex items-center gap-2 border-t border-slate-200 px-5 py-3">
			...
			>
				{submitting ? 'Staging…' : 'Stage uplink'}
			</button>
		</footer>
	</div>
</div>
