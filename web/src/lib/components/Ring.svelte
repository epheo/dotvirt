<script lang="ts">
	import { fmtUsage } from '$lib/format';
	import { usageLevelColor } from '$lib/status';
	import Sparkline from './Sparkline.svelte';

	let {
		label,
		used,
		total,
		allocated = 0,
		unit,
		color = 'var(--chart-1)',
		spark = [],
	}: {
		label: string;
		used: number;
		total: number;
		allocated?: number;
		unit: 'bytes' | 'cores';
		color?: string;
		spark?: number[];
	} = $props();

	const R = 38;
	const SW = 9;
	const C = 2 * Math.PI * R;
	const usedFrac = $derived(total > 0 ? Math.min(1, used / total) : 0);
	const allocFrac = $derived(total > 0 ? Math.min(1, allocated / total) : 0);
	const ringColor = $derived(usageLevelColor(usedFrac * 100, color));
</script>

<div class="flex flex-col items-center">
	<svg width="96" height="96" viewBox="0 0 96 96">
		<!-- capacity track. var() colors need style:, not SVG presentation attrs. -->
		<circle cx="48" cy="48" r={R} fill="none" style:stroke="var(--chart-track)" stroke-width={SW} />
		<!-- allocated/committed segment (faint), under the used arc -->
		{#if allocated > 0}
			<circle
				cx="48"
				cy="48"
				r={R}
				fill="none"
				style:stroke="var(--chart-track-strong)"
				stroke-width={SW}
				stroke-dasharray="{allocFrac * C} {C}"
				transform="rotate(-90 48 48)"
			/>
		{/if}
		<!-- used arc -->
		<circle
			cx="48"
			cy="48"
			r={R}
			fill="none"
			style:stroke={ringColor}
			stroke-width={SW}
			stroke-linecap="round"
			stroke-dasharray="{usedFrac * C} {C}"
			transform="rotate(-90 48 48)"
		/>
		{#if total === 0 && used === 0}
			<!-- Zero-of-zero is absence of data (no guest agent, empty scope), not
			     a measurement: "0 B of 0 B" invited reading it as a full disk. -->
			<text x="48" y="52" text-anchor="middle" class="fill-ink-faint text-[10px]">no data</text>
		{:else}
			<text x="48" y="46" text-anchor="middle" class="fill-ink text-[13px] font-semibold"
				>{fmtUsage(unit, used)}</text
			>
			<text x="48" y="60" text-anchor="middle" class="fill-ink-faint text-[9px]"
				>of {fmtUsage(unit, total)}</text
			>
		{/if}
	</svg>
	<div class="mt-0.5 text-xs font-medium text-ink-soft">{label}</div>
	{#if allocated > 0}
		<div class="text-[10px] text-ink-faint">{fmtUsage(unit, allocated)} allocated</div>
	{/if}
	{#if spark.length > 1}<div class="mt-0.5">
			<Sparkline values={spark} color={ringColor} width={84} height={16} />
		</div>{/if}
</div>
